...
export default function ActionFormPage() {
  ...
  const saveMutation = useAppMutation(
    async (payload: { CLAVE: string; NOMBRE: string; ACTIVO: number }) => {
      if (isEdit && id) return ActionsService.update(Number(id), payload);
      return ActionsService.create(payload);
    },
    {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await ActionsService.getById(Number(id));
  ...
  const validate = () => {
    ...
    if (!form.NOMBRE.trim()) {
      errors.NOMBRE = "El nombre de la acción es obligatorio";
    }

    if (!["0", "1"].includes(String(form.ACTIVO))) {
      errors.ACTIVO = "Debes seleccionar si la acción está activa";
    }
  ...
  if (loading) {
    return <LoaderOverlay label="Cargando acción..." />;
  }

  return (
    <Box>
      <PageHeader
        title={isEdit ? "Editar acción" : "Crear acción"}
        subtitle="Captura la información de la acción."
      />
      ...
      >
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <TextField
              ...
              helperText={
                submitted
                  ? errors.CLAVE || " "
                  : "Ejemplo: VER, CREAR, EDITAR, ELIMINAR"
              }
            />
          </Grid>
        ...
        >
          ...
          <Button variant="outlined" onClick={() => navigate("/actions")}>
            Cancelar
          </Button>
        </Box>
      </Paper>
